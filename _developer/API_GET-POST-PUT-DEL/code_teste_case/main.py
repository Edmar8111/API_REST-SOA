...
class Item(BaseModel): #INSTANCIAR A REQUISIÇÃO DA FUNÇÃO CREATE_ITEM PARA O DB 
    name:str
...
async def read_items(
    q: Annotated[str | None, 
                 Query(
                    title="Query string",description="This is an description return", 
                    min_length=3, alias="item-query", deprecated=False, include_in_schema=False
...
async def read_item(
    id:Annotated[str|None, AfterValidator(check_valid_id)]=None, include_in_schema=False,
):
    if id:
        item=data.get(id)
    else:
